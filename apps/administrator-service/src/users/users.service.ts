...
import { CreateAuthUserRequest } from '@app/common';
...
export class UserService {
  ...
  async createUserAndOrg(request: CreateUserAndOrgRequest) {
    const { email, organizationName, fullName, password } = request;
    this.logger.log(`Starting transaction for email: ${email}`);

    try {
      const res = await this.prismaService.$transaction(async (tx) => {
        const authReq: CreateAuthUserRequest = { email, password };
        const authUser = await this.authService.createAuthUser(authReq);
        this.logger.log(`Step 1/5: Authuser created`);
        ...
        this.logger.log(`Step 1/5: User created: ${newUser.id}`);
        ...
        this.logger.log(`Step 2/5: Org created: ${newOrganization.id}`);
        ...
        this.logger.log(`Step 3/5: Membership establised: ${newMembership.id}`);
        ...
        this.logger.log(
          `Step 4/5: Default org set for user ${newUser.id} -> ${newOrganization.id}`,
        );
